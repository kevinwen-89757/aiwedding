...
export function AdminOrderActions({ orderId, hasGenerated, status, hasThemes, adminNote: generationNote, updatedAt }: { orderId: string; hasGenerated: boolean; status: OrderStatus; hasThemes: boolean; adminNote?: string | null; updatedAt?: string }) {
  const router = useRouter();
  const [busy, setBusy] = useState("");
  const [error, setError] = useState("");
  const [adminNoteValue, setAdminNoteValue] = useState("");
  const [rejectReason, setRejectReason] = useState("");
  const hasActiveTask = Boolean(generationNote?.match(/task_id|任务轮询中/));
