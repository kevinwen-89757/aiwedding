...
function progressInfo(order: Awaited<ReturnType<typeof getLocalOrder>>, normalized: string) {
  ...
  if (normalized === "pending_selection") return { progress: 100, stageText: "预览图已生成，可以开始选片", helperText: "预览图已生成，可以开始选片。", canAutoRefresh: false };
  if (normalized === "generating") {
    const progress = generatedCount > 0 ? Math.min(90, 70 + generatedCount) : hasActiveTask ? 70 : 60;
    if (generatedCount > 0) return { progress, stageText: "已生成部分预览图，剩余图片还在处理中", helperText: `已生成部分预览图，剩余图片还在处理中。目前已生成 ${generatedCount} 张，页面会自动刷新进度。`, canAutoRefresh: true };
    if (hasActiveTask) return { progress, stageText: "已进入生成队列，系统正在查询生成结果", helperText: "已进入生成队列，系统正在查询生成结果。你可以稍后回来查看，页面会自动刷新进度。", canAutoRefresh: true };
    return { progress, stageText: "AI 正在生成高清婚纱写真预览，当前预计生成 5 张样片", helperText: "预计需要 2-5 分钟，请稍后刷新页面查看结果。", canAutoRefresh: true };
  }
