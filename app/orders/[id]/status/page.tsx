...
function progressInfo(order: Awaited<ReturnType<typeof getLocalOrder>>, normalized: string, plannedCount?: number) {
  ...
  const plannedText = plannedCount ? `（共 ${plannedCount} 张）` : "";
  ...
  if (normalized === "generating") {
    if (generatedCount > 0) return { progress: Math.min(95, Math.round(generatedCount / (plannedCount || generatedCount) * 80) + 10), stageText: "正在生成预览图中", helperText: `已生成 ${generatedCount}${plannedText}，系统自动每 60 秒更新进度，请耐心等待。`, queueText: "正在持续生成中，无需重复提交订单，页面会自动刷新。", canAutoRefresh: true };
