...
export async function apimartCreateGenerationTask(input: { prompt: string; uploadedImageUrls: string[]; aspectRatio?: string | null; resolution?: string | null }) {
  const imageUrls = input.uploadedImageUrls.filter((url) => url.trim());
  if (imageUrls.length < 1) throw new Error("APIMart 生成任务缺少参考图 URL。");
  const resolution = input.resolution ?? appConfig.apimartResolution;
