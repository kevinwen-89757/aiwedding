...
export async function createWatermarkedPreviewBuffer(input: Buffer) {
  ...
  const svg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg"><style>text{font-family:Arial,sans-serif;font-size:42px;font-weight:700;fill:rgba(255,255,255,0.4);letter-spacing:2px}</style><g transform="rotate(-28 ${width / 2} ${height / 2})">${marks}</g></svg>`;
